...
void s3MouseEvent::print(int state) const
{
    s3Log::debug("%s type: %s, pos: [%d, %d], offset: [%d, %d], scroll: %.1f, control: %s, shift: %s\n",
        ...
        s3MouseTypeToString(type).c_str(), x, y, offsetX, offsetY, scroll, s3BoolToString(control).c_str(), s3BoolToString(shift).c_str());
}
...
void s3KeyEvent::print(int state) const
{
    if(key == 0)
        ...
        s3KeyTypeToString(keyCode).c_str(), s3BoolToString(control).c_str(), s3BoolToString(shift).c_str(), s3BoolToString(alt).c_str());
    else
        s3Log::debug("%s code: %s, key: %c, control: %s, shift: %s, alt: %s\n", 
        s3KeyStateToString((s3KeyState)state).c_str(), 
        s3KeyTypeToString(keyCode).c_str(), (uint8) key, s3BoolToString(control).c_str(), s3BoolToString(shift).c_str(), s3BoolToString(alt).c_str());
}
